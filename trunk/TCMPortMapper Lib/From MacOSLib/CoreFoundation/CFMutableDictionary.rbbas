...
Inherits CFDictionary
	#tag Method, Flags = &h0
		Sub Constructor()
		  #if targetMacOS
		    soft declare function CFDictionaryCreateMutable lib CarbonLib (allocator as Ptr, capacity as Integer, keyCallbacks as Ptr, valueCallbacks as Ptr) as Ptr
	...
	#tag Method, Flags = &h0
		Sub Constructor(theDictionary as CFDictionary)
		  #if TargetMacOS
		    soft declare function CFDictionaryCreateMutableCopy lib CarbonLib (allocator as Ptr, capacity as Integer, theDict as Ptr) as Ptr
		    
		    if not (theDictionary is nil) then
		      super.Constructor CFDictionaryCreateMutableCopy(nil, 0, me.Reference), true
		    end if
		  #endif
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Clear()
		  #if targetMacOS
		    soft declare sub CFDictionaryRemoveAllValues lib CarbonLib (theDict as Ptr)
	...
	#tag Method, Flags = &h0
		Sub Remove(key as CFType)
		    ...
		    soft declare sub CFDictionaryRemoveValue lib CarbonLib (theDict as ptr, key as Ptr)
	...
	#tag Method, Flags = &h0
		Sub Value(key as CFType, assigns value as CFType)
		  #if targetMacOS
		    soft declare sub CFDictionarySetValue lib CarbonLib (theDict as Ptr, key as Ptr, value as Ptr)
